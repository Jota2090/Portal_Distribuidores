...
class Cpdf
{
                ...
                if (count($o['info']['pages'])) {
                    ...
                    if ((isset($o['info']['fonts']) && count($o['info']['fonts'])) || isset($o['info']['procset'])) {
                        ...
                        if (isset($o['info']['mediaBox'])) {
                            $tmp = $o['info']['mediaBox'];
                            $res .= "\n/MediaBox [" . sprintf('%.3f', $tmp[0]) . ' ' . sprintf('%.3f', $tmp[1]) .
                                ' ' . sprintf('%.3f', $tmp[2]) . ' ' . sprintf('%.3f', $tmp[3]) . ']';
                        }
                    }
                    $res .= "\n >>\nendobj";
                } else {
    ...
    function o_font($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                $this->objects[$id] = array('t' => 'font', 'info' => array('name' => $options['name'],
                            'SubType' => 'Type1'));
                ...
                if (isset($options['differences'])) {
                    ...
                    $this->o_pages($this->currentNode, 'font', array('fontNum' => $fontNum, 'objNum' =>
                            $id));
                break;
            case 'add':
                foreach ($options as $k => $v) {
    ...
    function o_procset($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                $this->objects[$id] = array('t' => 'procset', 'info' => array('PDF' => 1, 'Text' =>
                            1));
    ...
    function o_info($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                $this->infoObject = $id;
                $date = 'D:' . date('Ymd');
                $this->objects[$id] = array('t' => 'info', 'info' => array('Creator' =>
                            'R and OS php pdf writer, http://www.ros.co.nz', 'CreationDate' => $date));
                break;
            case 'Title':
            case 'Author':
            case 'Subject':
            ...
            case 'new':
                ...
                if (!isset($options['type']) || $options['type'] == 'jpg') {
                    ...
                    if ($options['type'] == 'png') {
                        ...
                        $this->objects[$id]['info']['DecodeParms'] = '<< /Predictor 15 /Colors ' . $options['ncolor'] .
                            ' /Columns ' . $options['iw'] . ' /BitsPerComponent ' . $options['bitsPerComponent'] .
                            '>>';
    ...
    function o_encryption($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                ...
                $pad = chr(0x28) . chr(0xBF) . chr(0x4E) . chr(0x5E) . chr(0x4E) . chr(0x75) .
                    chr(0x8A) . chr(0x41) . chr(0x64) . chr(0x00) . chr(0x4E) . chr(0x56) . chr(0xFF) .
                    chr(0xFA) . chr(0x01) . chr(0x08) . chr(0x2E) . chr(0x2E) . chr(0x00) . chr(0xB6) .
                    chr(0xD0) . chr(0x68) . chr(0x3E) . chr(0x80) . chr(0x2F) . chr(0x0C) . chr(0xA9) .
     ...
     * @access private
     */
    function newDocument($pageSize = array(
        0,
        0,
        612,
        792))
    ...
    function openFont($font)
    {
        ...
        if (file_exists($dir . 'php_' . $name . '.afm')) {
            ...
            if (!isset($this->fonts[$font]['_version_']) || $this->fonts[$font]['_version_'] <
                1) {
    ...
    function setCurrentFont()
    {
        if (strlen($this->currentBaseFont) == 0) {
            ...
            $this->selectFont('./recursos/fonts/Helvetica.afm');
        }
    ...
    {
        $this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x1) . ' ' .
            sprintf('%.3f', $y1) . ' m ' . sprintf('%.3f', $x2) . ' ' . sprintf('%.3f', $y2) .
            ' l S';
    }
    ...
    function curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3)
    {
        ...
        $this->objects[$this->currentContents]['c'] .= ' ' . sprintf('%.3f', $x2) . ' ' .
            sprintf('%.3f', $y2) . ' ' . sprintf('%.3f', $x3) . ' ' . sprintf('%.3f', $y3) .
            ' c S';
    }
     ...
     */
    function partEllipse($x0, $y0, $astart, $afinish, $r1, $r2 = 0, $angle = 0, $nSeg =
        8)
    {
     ...
     */
    function filledEllipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart =
        0, $afinish = 360)
    {
        return $this->ellipse($x0, $y0, $r1, $r2 = 0, $angle, $nSeg, $astart, $afinish,
            1, 1);
    }
     ...
     */
    function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart = 0, $afinish =
        360, $close = 1, $fill = 0)
    {
        if ($r1 == 0) {
            return;
        ...
        for ($i = 1; $i <= $nSeg; $i++) {
            ...
            $this->objects[$this->currentContents]['c'] .= ' ' . sprintf('%.3f', ($a1 - $c1 *
                $dtm)) . ' ' . sprintf('%.3f', ($b1 - $d1 * $dtm)) . ' ' . sprintf('%.3f', $a1) .
                ' ' . sprintf('%.3f', $b1) . ' c';
            $a0 = $a1;
            $b0 = $b1;
            $c0 = $c1;
    ...
    {
        $this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x1) . ' ' .
            sprintf('%.3f', $y1) . ' ' . sprintf('%.3f', $width) . ' ' . sprintf('%.3f', $height) .
            ' re f';
    }
    ...
    function stream($options = '')
    {
        ...
        if (isset($options['descarga'])) {
            header("Content-type: application/force-download");
            header("Content-Disposition: filename=" . $options['descarga']);
            ...
            $fileName = (isset($options['Content-Disposition']) ? $options['Content-Disposition'] :
                'file.pdf');
     ...
     * @access private
     */
    function PRVTcheckTextDirective1(&$text, $i, &$f, $final, &$x, &$y, $size = 0, $angle =
        0, $wordSpaceAdjust = 0)
    {
        $directive = 0;
        $j = $i;
                        ...
                        case 'i':
                            ...
                            if ($text[$j] == '>') {
                                ...
                                if ($p !== false) {
                                    ...
                                    $this->currentTextState = substr($this->currentTextState, 0, $p) . substr($this->
                                        currentTextState, $p + 1);
                                }
                                $directive = $j - $i + 1;
                            }
                            break;
    ...
    function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0)
    {
        ...
        if ($wordSpaceAdjust != 0 || $wordSpaceAdjust != $this->wordSpaceAdjust) {
            ...
            $this->objects[$this->currentContents]['c'] .= ' ' . sprintf('%.3f', $wordSpaceAdjust) .
                ' Tw';
        }
        $len = strlen($text);
        $start = 0;
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($directive) {
                ...
                if ($f) {
                    ...
                    $xp = $x;
                    $yp = $y;
                    $directive = $this->PRVTcheckTextDirective1($text, $i, $f, 1, $xp, $yp, $size, $angle,
                        $wordSpaceAdjust);
     ...
     */
    function addTextWrap($x, $y, $width, $size, $text, $justification = 'left', $angle =
        0, $test = 0)
    {
    ...
    function addPngFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        if (!$error) {
            $header = chr(137) . chr(80) . chr(78) . chr(71) . chr(13) . chr(10) . chr(26) .
                chr(10);
        ...
        if (!$error) {
            if ($info['colorType'] != 2 && $info['colorType'] != 0 && $info['colorType'] !=
                3) {
                $error = 1;
        ...
        $this->objects[$this->currentContents]['c'] .= "\nq";
        $this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $w) .
            " 0 0 " . sprintf('%.3f', $h) . " " . sprintf('%.3f', $x) . " " . sprintf('%.3f',
            $y) . " cm";
        $this->objects[$this->currentContents]['c'] .= "\n/" . $label . ' Do';
        $this->objects[$this->currentContents]['c'] .= "\nQ";
    }
